...
public class RedisConfig {
    ...
    public CacheManager cacheManager(RedisConnectionFactory redisConnectionFactory) {
        ...
        // 팔로잉 목록 캐시 (5분 - 자주 변경될 수 있으므로)
        cacheConfigurations.put("followings", config.entryTtl(Duration.ofMinutes(5)));

        // 팔로워 목록 캐시 (5분 - 자주 변경될 수 있으므로)
        cacheConfigurations.put("followers", config.entryTtl(Duration.ofMinutes(5)));
        
        // 팔로우 관계 캐시 (5분)
        cacheConfigurations.put("followRelation", config.entryTtl(Duration.ofMinutes(5)));
